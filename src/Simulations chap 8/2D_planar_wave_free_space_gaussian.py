...
t0 = 10 * dt
spread = 2 * dt
theta = 30 * np.pi / 180  # Angle of incidence in radians
delay = dx * np.sin(theta) / c
# Create Fields
...
fig, ax = plt.subplots()
norm = Normalize(vmin=-0.6, vmax=0.6)  # Initial normalization (vmax will be updated dynamically)
    ...
    if frame > 0:
        if frame < 40:
            for m in range(10, 150):
                t = frame * dt
                E[250, m] = np.exp(-((t - t0 - m * delay) / spread) ** 2)
...
# Save the animation
ani.save("2D__planar_wave_test_p1.mp4", fps=45) #Must save before plt.show() but then additional waiting time
