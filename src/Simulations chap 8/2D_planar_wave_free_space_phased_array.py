...
M = 500 # Number of x steps
N = 500 # Number of y steps
    ...
    if frame > 0:
        if frame < 100 :
            ...
            for m in range(50, 200, 1):
                E[250, m] = np.sin(omega * frame * dt + m*delta_phi)  # Sine wave source
...
# Save the animation
ani.save("2D__planar_wave_test_final.mp4", fps=45) #Must save before plt.show() but then additional waiting time
