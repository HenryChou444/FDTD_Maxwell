...
# Create E field
E = np.zeros((2, M))  # Electric field, last sample is (M-1)
B = np.zeros((2, M-1))  # Magnetic field, last sample is (M-2)
E_boundary = np.zeros((1, 2))  # Boundary conditions for E field
...
def update(frame):
    #   # Boundary conditions
    if frame > a - 1 :
        E[1, 0] = E_boundary[0, 0] 
        E[1, M-1] =  E_boundary[0,1]
    if frame > 0:
        for m in range(1, M - 1): #1 compris, M-1 exclu
            E[1, m] = E[0, m] + 1/a *(B[0, m]- B[1, m-1]) - (J[frame,m]) #Normalized J
        ...
        if frame < Q-1: #Ne calcule pas la dernière ligne de B (pas utile pour trouver E)
            for m in range(0, M - 1): #0 compris, M-1 exclu
                B[1, m] = B[0,m] + 1/a *(E[1, m+1] - E[1, m]) 
                
        E_boundary[0, 0] = E[0, 1]  # Update left boundary
        E_boundary[0, 1] = E[0, M-2]  # Update right boundary
        E[0, :] = E[1, :] #Previous values
        B [0, :] = B[1, :]  #Previous values
    # Update the title with the current frame number   
    animated_source.set_data(x, E[1, :]) # Update E field
    ax.set_title(f"1D FDTD - Frame {frame}")
    #print(f"Updated frame {frame}")
    return animated_source, ax.title

# Create the animation
ani = FuncAnimation(fig, update, frames=Q, interval=15, blit=False)
